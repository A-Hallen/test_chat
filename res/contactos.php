...
function sala($conection, $sessionID){
    $salaQuery = mysqli_query($conection, "SELECT `id_sala` FROM `sala_usuario` WHERE id_usuario = '$sessionID'");
    $i = 0;
    $a = array();
...
$id_consulta     = mysqli_query($conection, "SELECT  id_sala FROM sala_usuario WHERE id_usuario = '$sessionID'");
//test
$indice = 0;
while ($row = mysqli_fetch_array($id_consulta)) {
    //Buscamos el indice de las salas en las que se encuentra el cliente    $sala
    $sala_array = sala($conection, $sessionID);
    ...
    if ($salaType == 1) {
        ...
        $contactoQuery = mysqli_query($conection, "SELECT `id_usuario` FROM `sala_usuario` WHERE id_usuario != '$sessionID' AND id_sala = '$sala'");
        ...
        $nombre_contactos = mysqli_query($conection, "SELECT Nombre FROM usuarios WHERE Session_id = '$contacto'");
